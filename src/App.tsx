...
const N8N_BASE = 'https://stamman.app.n8n.cloud'; // TODO replace with your n8n domain
    ...
    <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-slate-200">
      <div className="max-w-6xl mx-auto px-4 py-3 flex justify-between items-center">
        ...
        <div className="flex items-center gap-6 text-sm">
          ...
          {isAuthenticated && <a href="#automations" className="hover:text-slate-900 text-slate-600">My Automations</a>}
          {!isAuthenticated ? (
            <button onClick={() => loginWithRedirect()} className="px-4 py-1.5 rounded-xl text-white bg-sky-600 hover:bg-sky-700 transition">Log in / Sign up</button>
          ) : (
            <button onClick={() => logout()} className="px-4 py-1.5 rounded-xl text-slate-700 border border-slate-300 hover:bg-slate-50 transition">Log out</button>
          )}
        </div>
      </div>
    </nav>
    ...
    <section id="services" className="py-20 max-w-6xl mx-auto px-4">
      ...
      <div className="mt-8 grid md:grid-cols-2 gap-8">
        <div className="p-6 bg-white rounded-3xl border border-slate-200 shadow hover:shadow-lg transition">
          ...
          <p className="mt-2 text-slate-600">Answers calls, books to Google Calendar, handles FAQs. 24/7 availability.</p>
          <div className="mt-4 flex gap-3">
            <a href="#pricing" className="px-4 py-2 rounded-xl bg-sky-600 text-white hover:bg-sky-700 transition">Test your own AI receptionist for free</a>
            <a href="#pricing" className="px-4 py-2 rounded-xl border border-slate-300 bg-white hover:shadow transition">Buy</a>
          </div>
        </div>
      </div>
    </section>
...
function Pricing(){
  function handleBuyClick(e){ e.preventDefault(); alert('Checkout will be enabled after PayPal is connected.'); }
  return (
    <section id="pricing" className="py-20 bg-sky-50">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="mt-8 grid md:grid-cols-3 gap-8">
          <div className="p-6 bg-white rounded-3xl border border-slate-200 shadow hover:shadow-lg transition">
            <h3 className="text-xl font-semibold text-slate-900">AI Receptionist</h3>
            <p className="mt-2 text-3xl font-bold text-slate-900">$299<span className="text-lg font-normal">/mo</span></p>
            <div className="mt-4">
              <a onClick={handleBuyClick} href="#" className="px-4 py-2 rounded-xl bg-sky-600 text-white hover:bg-sky-700 transition">Buy</a>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <section id="automations" className="py-20 max-w-6xl mx-auto px-4">
      ...
      <div className="mt-6 p-6 border rounded-3xl bg-white shadow text-slate-600">
        ...
        <div className="mt-3"><a href="#services" className="px-4 py-2 rounded-xl bg-sky-600 text-white hover:bg-sky-700 transition">Buy</a></div>
      </div>
    </section>
  );
}
...
function AppInner(){ return (<div><Navbar/><Hero/><Services/><Pricing/><Automations/><Footer/></div>); }
